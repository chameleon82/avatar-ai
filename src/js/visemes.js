...
(function () {
    ...
    function detectFromSpectrum(frequencyData, sampleRate, fftSize) {
        ...
        // Your logs for sustained "А": centroid ~620–660, midRatio ~0.65–0.73, lowRatio ~0.25–0.33, high/air tiny.
        // Prefer 'aa' in that pocket to avoid mapping "А" to 'E'.
        if (
        ...
        // "aa" guard for mics/voices where sustained "A" lands in mid centroids.
        // Prevents "АААА" from drifting into 'E'. Keep this fairly strict.
        if (
        ...
        // Stronger "A" guard: if low band clearly dominates, treat as 'aa' even if centroid is in the E-region.
        // This is tuned for cases where the mic/voice makes "А" look like mid-vowel centroids.
        if (
            centroid < 2100 &&
            lowRatio > 0.34 &&
            lowRatio > midRatio &&
    ...
    function createOutputTracker({
        ...
        maxDecibels = -10,
        silenceRms = 0.010,
        // Stabilization to avoid rapid E/O/aa flicker on short vowels
        minHoldMs = 90,
        confirmFrames = 3,
        intervalMs = 20,
        debug = false,
                ...
                if (activeUntilSec > 0 && now <= activeUntilSec) {
                    ...
                    if (true) {
                        ...
                        if (nowMs - lastDebugTs >= debugIntervalMs) {
                            ...
                            const low = bandEnergy(freq, 80, 400, ctx.sampleRate, analyser.fftSize);
                            ...
                            const total = low + mid + high + air;
                            const centroid = spectralCentroidHz(freq, 80, 6000, ctx.sampleRate, analyser.fftSize);
